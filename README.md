...
4. Extract it and run either the `.exe` installer or `.msi` package.
...
Push to `main`, create a version tag, or run the workflow manually from the Actions tab. The GitHub-hosted Windows builder installs the toolchain and produces NSIS (`.exe`) and MSI installers. No build setup is required on the machine that downloads the result.
